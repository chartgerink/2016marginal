ve##Tables for marginal significance paper, data here: https://osf.io/28gxz/ and here: https://github.com/chartgerink/2016marginal/tree/master/data
...
#------------------------------------------
#Add variable searching for marginal results only pre-  p-values
dat2$marginalpre <- grepl("margin|approach", dat2$pre)

#percentages marginally significant when searching only pre p-values
marg.overallpre <- 100*(sum(dat2$marginalpre) / length(dat2$result))
marg.clinicalpre <- 100*(sum(dat2$marginalpre[dat2$Clinical.Psychology == 1]) / length(dat2$result[dat2$Clinical.Psychology == 1]))
marg.cognitivepre <- 100*(sum(dat2$marginalpre[dat2$Neuroscience...Cognition == 1]) / length(dat2$result[dat2$Neuroscience...Cognition == 1]))
marg.developmentalpre <- 100*(sum(dat2$marginalpre[dat2$Developmental.Psychology == 1]) / length(dat2$result[dat2$Developmental.Psychology == 1]))
marg.educationalpre <- 100*(sum(dat2$marginalpre[dat2$Educational.Psychology..School.Psychology...Training == 1]) / length(dat2$result[dat2$Educational.Psychology..School.Psychology...Training == 1]))
marg.experimentalpre <- 100*(sum(dat2$marginalpre[dat2$Basic...Experimental.Psychology == 1]) / length(dat2$result[dat2$Basic...Experimental.Psychology == 1]))
marg.forensicpre <- 100*(sum(dat2$marginalpre[dat2$Forensic.Psychology == 1]) / length(dat2$result[dat2$Forensic.Psychology == 1]))
marg.healthpre <- 100*(sum(dat2$marginalpre[dat2$Health.Psychology...Medicine == 1]) / length(dat2$result[dat2$Health.Psychology...Medicine == 1]))
marg.organizationalpre <- 100*(sum(dat2$marginalpre[dat2$Industrial.Organizational.Psychology...Management == 1]) / length(dat2$result[dat2$Industrial.Organizational.Psychology...Management == 1]))
marg.socialpre <- 100*(sum(dat2$marginalpre[dat2$Social.Psychology...Social.Processes == 1]) / length(dat2$result[dat2$Social.Psychology...Social.Processes == 1]))

marg.difference <- rep(NA, 10)
marg.pre <- c(marg.overallpre, marg.clinicalpre, marg.cognitivepre, marg.developmentalpre, marg.educationalpre,
              marg.experimentalpre, marg.forensicpre, marg.healthpre, marg.organizationalpre, marg.socialpre)
...
for (diff in 1:10) {
  marg.difference[diff] <- marg[diff] - marg.pre[diff]  
}
...
marginal_list <- list(entries.original = prettyNum(nrow(original), big.mark = ",", preserve.width = "none"), 
                      ...
                      difarticles = length(setdiff(unique(dat$doi[dat$Core.of.Psychology == 1]), noncore)),
