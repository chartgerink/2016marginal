...
Finally, we excluded _p_-values outside of the range of .05 - .1 and created a test sample. Limiting the dataset to _p_-values $.05 < p \leq.1%$, resulted in a final sample consisting of `r marginal$entries.final` (`r marginal$entries.finalpercent`% of total) _p_-values. No additionl values were excluded from the dataset beyond those described under the current header. From the finished dataset, we drew a stratified random sample (by journal) of 6% for testing and pre-registering analytic code. For our analyses we used the full final dataset (including the test sample data).
...
Due to using a non-random sample (only APA-articles available in HTML format at the time of download), we present only descriptive statistics. Consequently, we did not perform a power analysis, and perform no statistical significance testing. Prior to the analysis, we disaggregated all p-values and grouped them by year and psychological subfield (or in the case of JPSP and DP, by journal). We describe percentages of 'marginally significant' results across years and subfields, and for JPSP/DP. To aid interpretation we estimate simple linear regressions using least squares estimation, and report _b_-values and the coefficients of determination. Our dependent variable is the percentage of_p_-values ($.05 < p \leq.1%$) reported as marginally significant, and our independent variable is the year (range 1985 - 2016 including endpoints) of publication of the articles from which the _p_-values were extracted. We additionally report average percentage of _p_-values ($.05 < p \leq.1%$) reported as marginally significant across the years for each subfield, as well as for JPSP and DP (see table 1 and 2). We will conduct no other preregistered data analyses, and should any additional analyses be conducted we will accordingly label them as exploratory.
