...
That our estimates were higher than those of @10.1177/0956797616645672 has three likely explanations. It may be: 1) due to @10.1177/0956797616645672 only observing a few select years, 2) due to a difference in denominators between @10.1177/0956797616645672's DV and ours, or 3) due to a difference in numerators between the DVs. We examine these possibilities below.

It may be that @10.1177/0956797616645672 coincidentally only examined years with lower percentages of 'marginally significant' results. Of the five years examined by @10.1177/0956797616645672 our data includes three (1990, 2000, 2010; see Figure 2). Using only these three years we estimated the percent 'marginally significant' _p_-values in JPSP and DP. Comparing this estimate for JPSP (`r marginal$marg.jpsp.1990.2010`%) with that of @10.1177/0956797616645672 for JPSP (`r marginal$pritschet.jpsp`%), the difference seems to largely disappear. While our estimate is still about a percentage point higher, @10.1177/0956797616645672 report low proportions of 'marginally significant' results for the years not included in our sample (1970/1980) which might explain the remaining difference. The difference in estimates for JPSP thus appears to be explained by which years were examined. The same is not true of DP where the difference between our estimate and @10.1177/0956797616645672's remains large (`r marginal$marg.dp.1990.2010`% versus `r marginal$pritschet.dp`%) and practically unchanged from the estimate based on all 32 years in our sample (`r marginal$marg.dp`%). 

The denominator of @10.1177/0956797616645672's DV may be inflated compared to our denominator. If the number of articles lacking 'marginally significant' results is higher than the number of _p_-values ($.05 < p \leq.1$) not reported as marginally significant, then the DV of @10.1177/0956797616645672 will result in lower estimates. The denominator for @10.1177/0956797616645672's DV is the number of articles containing _p_-values and ours is the number of _p_-values between .05 and .1. In our data, DP has `r marginal$no.marg.a.dp` articles without 'marginally significant' results and `r marginal$amarg.dp` with at least one. DP also has `r marginal$no.marg.p.dp` _p_-values ($.05 < p \leq.1$) not reported as marginally significant and at least as many 'marginally significant' _p_-values as there are articles containing such results. Even in the extreme case of there being only one 'marginally significant' _p_-value in each article containing 'marginally significant' results (the numerator thus being the same between DVs), our DV would still give an estimate for DP about 7 percentage points higher (`r marginal$extreme.dp`%) than the estimate using @10.1177/0956797616645672's article level DV (`r marginal$dp.percent.article`%).

The numerator of @10.1177/0956797616645672's DV may be deflated compared to ours due to not taking into account dependencies at the article level in 'marginally significant' _p_-values. This may partly explain their lower estimates. In DP there are `r marginal$marg.dp.no` 'marginally significant' _p_-values between .05 and .1 (our numerator), but only `r marginal$amarg.dp` articles containing at least one 'marginally significant' result [@10.1177/0956797616645672's numerator]. Holding the non-'marginally significant' parts of the two DV denominators the same for DP (e.g equal to the number of articles without 'marginally significant' results), this effect leads to a higher estimate for our DV by about 10 percentage points (`r marginal$dp.partly.equal.denom`% vs `r marginal$dp.percent.article`%). For DP, the combined effect of differences in the numerator and denominator between the DVs is about 17% percentage points. This is actually about 2 percentage points more than the difference in estimates between @10.1177/0956797616645672 and us for this journal (`r marginal$pritschet.dp`% versus `r marginal$marg.dp`%). This might be an indication that our automated extraction of 'marginally significant' _p_-value misses a few 'marginally significant' results that would have been found with manual extraction.
...
```
We have examined the incidence of 'marginally significant' results in psychology and its disciplines between 1985 and 2016. @10.1177/0956797616645672 previously examined the prevalence of 'marginal significance' in three psychological journals: JPSP (representing social psychology), DP (representing developmental psychology) and 'Cognitive Psychology'(representing cognitive psychology). However, they did not take into account the increasing number of _p_-values over time, nor the differences in _p_-values between disciplines. We have re-examined their claims that 'marginally significant' results have become more common in social, developmental and cognitive psychology; and that they are more common in social than in developmental or cognitive psychology. We have also examined the incidence of 'marginally significant' results in 6 additional psychological disciplines beyond these three. To do so we have examined 42,504 _p_-values ($.05 < p \leq.1$) extracted from 70 APA journals using regular expressions. We searched the 400 characters surrounding a _p_-value for indicators of marginal significance and reported the percentages of _p_-values between .05 and .1 that were indicated as marginally significant.
...
All disciplines described a downward trend in the percentage of results reported as marginally significant between 1985 and 2016. Controlling for the increasing numbers of _p_-values across the years, the positive trends reported by @10.1177/0956797616645672 for cognitive, developmental and social psychology have thus disappeared. On the other hand, JPSP, which @10.1177/0956797616645672 used to represent social psychology, continues to show a positive trend. This illustrates the problem with using a single journal to represent entire psychological disciplines. The downward trend in psychology may reflect increasing awareness amongst researchers that _p_-values in the .05 - .1 range represent very weak evidence against the null [see e.g. @osf.io/preprints/psyarxiv/mky9j]. It may also be that percentages are decreasing due to increasingly stringent competition to publish and less leniency amongst editors for 'marginally significant' results [as previously suggested by @10.7717/peerj.1142]. Regardless of the reason, what matters is what happens to these studies instead, whether they end up in the file drawer, transformed into significant results [@10.1177/0956797611417632], or are published nonetheless, but without being called 'marginally significant'. We naturally hope for the latter.

Our results are qualified by two factors. First, it is likely that our use of automatic extraction has occasionally resulted in _p_-values being incorrectly interpreted as 'marginally significant' or vice versa. This first limitation is largely compensated by our second limitation. Matthew @Hankins has compiled a list of `r nrow(b)` ways that researchers have described results as marginally significant. Out of these, only `r sum(b$marg)` include the expressions "margin\*" or "approach\*", our indicators of 'marginal significance'. Although there is no telling how common the different expressions on Hankins' list are, our estimates of the proportions of 'marginally significant' results are nonetheless likely to be underestimates.
